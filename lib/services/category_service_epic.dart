      ...
      for (int i = 0; i < businessListFromFirebase.docs.length; i++) {
        ...
        if (!tmpBusinessIdList.contains(businessListFromFirebase.docs[i].id)) {
          ...
          if (store.state.area != null && store.state.area.areaId != null && store.state.area.areaId.isNotEmpty) {
            query = servicesFirebase.where("businessId", isEqualTo: businessListFromFirebase.docs[i].id).where("tag", arrayContains: store.state.area.areaId).where("serviceCrossSell", isEqualTo: true).limit(20);
          } else {
            query = servicesFirebase.where("businessId", isEqualTo: businessListFromFirebase.docs[i].id).where("serviceCrossSell", isEqualTo: true).limit(20);
          }
