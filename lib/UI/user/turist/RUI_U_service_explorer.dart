...
class _RServiceExplorerState extends State<RServiceExplorer> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      _orderStream = FirebaseFirestore.instance.collection("order").where("userId", isEqualTo: StoreProvider.of<AppState>(context).state.user.uid).where("date", isGreaterThanOrEqualTo: currentTime).limit(50).snapshots(includeMetadataChanges: true);

      ///TODO: Vedere se gestire la query con serviceCount>0
      //_categoryStream = FirebaseFirestore.instance.collection("category").where("serviceCount", isGreaterThan: 0).snapshots(includeMetadataChanges: true);
      _categoryStream = FirebaseFirestore.instance.collection("category").snapshots(includeMetadataChanges: true);
      ...
      _businessStream = FirebaseFirestore.instance.collection("business").where("draft", isEqualTo: false).snapshots(includeMetadataChanges: true);

      _bookingStream = FirebaseFirestore.instance.collection("booking").where("userEmail", arrayContains: StoreProvider.of<AppState>(context).state.user.email).where("status", isEqualTo: 'opened').where("end_date", isGreaterThanOrEqualTo: currentTime).orderBy("end_date", descending: true).snapshots(includeMetadataChanges: true);
    });
  ...
  void popularServiceScrollListener() {
    if (popularServiceScroller.offset >= popularServiceScroller.position.maxScrollExtent && !popularServiceScroller.position.outOfRange) {
    ...
    for (var z = 0; z < serviceListSnippetListState.length; z++) {
      for (var w = 0; w < serviceListSnippetListState[z].businessSnippet.length; w++) {
        for (var y = 0; y < serviceListSnippetListState[z].businessSnippet[w].serviceList.length; y++) {
          if (serviceId != null && categoryId != null && serviceListSnippetListState[z].businessSnippet[w].serviceList[y].serviceAbsolutePath.contains(serviceId) && serviceListSnippetListState[z].businessSnippet[w].serviceList[y].serviceAbsolutePath.contains(categoryId)) {
            sub = true;
          }
        }
      }
  ...
  ///Funzione ricerca tramite Algolia
  void newSearchCategory(List<CategoryState> rootCategoryList) async {
    //debugPrint('RUI_U_service_explorer => CATOGORY LIST: ${list.length}');
    ...
    if (_searchController.text.isNotEmpty) {
      //debugPrint('RUI_U_service_explorer => CATOGORY LIST: ${list.length}');
      rootCategoryList.forEach((element) {
        //debugPrint('RUI_U_service_explorer => ${element.name.toLowerCase()} - ${_searchController.text.toLowerCase().trim()} - ${element.name.toLowerCase().contains(_searchController.text.toLowerCase().trim())}');
        if (element.name.toLowerCase().contains(_searchController.text.toLowerCase().trim())) {
          tmpCategoryList.add(element);
        }
      });
    }
    ...
    if (_searchController.text.length >= 3) {
      ...
      setState(() {
        if (snap.hits.isNotEmpty) {
          ...
          Provider.of<Explorer>(context, listen: false).searchedList.add(tmpCategoryList);
          Provider.of<Explorer>(context, listen: false).searchedList.add(tmpServiceList);
        } else {
          Provider.of<Explorer>(context, listen: false).searchedList.add(tmpCategoryList);
        }
      });
                                              ...
                                              child: Align(
                                                ...
                                                child: IconButton(
                                                  ...
                                                  onPressed: () {
                                                    ...
                                                    FirebaseAnalytics().logEvent(name: 'cart_discover', parameters: {
                          ...
                          : AppBar(
                              ...
                              title: Container(
                                ...
                                child: TextFormField(
                                  ...
                                  onEditingComplete: () {
                                    FirebaseAnalytics().logEvent(name: 'search_discover', parameters: {'user_email': snapshot.user.email, 'date': DateTime.now().toString(), 'string_searched': _searchController.text});
                                    //debugPrint('RUI_U_service_explorer => done');
                                    FocusScope.of(context).unfocus();
                                    Provider.of<Explorer>(context, listen: false).searchedList.clear();
                                ...
                                children: [
                                  ...
                                  _searchController.text.isEmpty
                                      ? Column(
                                          ...
                                          children: [
                                            ///My bookings if user
                                            FirebaseAuth.instance.currentUser != null && FirebaseAuth.instance.currentUser.uid.isNotEmpty /*&& cards.isNotEmpty*/ ? StreamBuilder<QuerySnapshot>(
                                                                              ...
                                                                              Row(
                                                                                children: [
                                                                                  ...
                                                                                  Container(
                                                                                    margin: EdgeInsets.only(left: SizeConfig.safeBlockHorizontal * 3.5, top: SizeConfig.safeBlockVertical * 0.5),
                                                                                    child: Text(
                                                                                      sameMonth ? '${DateFormat('dd', Localizations.localeOf(context).languageCode).format(bookingState.start_date)} - ${DateFormat('dd MMMM', Localizations.localeOf(context).languageCode).format(bookingState.end_date)}' : '${DateFormat('dd MMM', Localizations.localeOf(context).languageCode).format(bookingState.start_date)} - ${DateFormat('dd MMM', Localizations.localeOf(context).languageCode).format(bookingState.end_date)}',
                                                                                      style: TextStyle(fontFamily: BuytimeTheme.FontFamily, color: BuytimeTheme.TextMedium, fontWeight: FontWeight.w400, fontSize: 16
                                          ...
                                          Colors.white,
                                                                            ...
                                                                            child: Container(
                                                                              ...
                                                                              child: CachedNetworkImage(
                                                                                //width: 125,
                                                                                imageUrl: businessState != null && businessState.logo != null ? businessState.logo : 'https://firebasestorage.googleapis.com/v0/b/buytime-458a1.appspot.com/o/general%2Fimage_placeholder_200x200_1000x1000.png?alt=media&token=082a1896-32d8-4750-b7cc-141f00bc060c',
                                                    ...
                                                    builder: (context, AsyncSnapshot<QuerySnapshot> orderSnapshot) {
                                                      ...
                                                      orderSnapshot.data.docs.forEach((element) {
                                                        ...
                                                        if ((order.progress == Utils.enumToString(OrderStatus.paid) ||
                                                            ...
                                                            /* ((order.itemList.first.date.isAtSameMomentAs(currentTime) ||
                                                               order.itemList.first.date.isBefore(currentTime)) && order.progress != Utils.enumToString(OrderStatus.paid))*/
                                                      ...
                                                      userOrderList.sort((a, b) => a.itemList.first.date.isBefore(b.itemList.first.date)
                                                          ...
                                                          : a.itemList.first.date.isAtSameMomentAs(b.itemList.first.date)
                                                              ? 0
                                                              : 1);
                                                      orderList.sort((a, b) => a.date.isBefore(b.date)
                                                          ? -1
                                                          : a.date.isAtSameMomentAs(b.date)
                                                              ? 0
                                                              : 1);
                                                      return userOrderList.isNotEmpty || orderList.isNotEmpty
                                                          ? Container(
                                                              margin: EdgeInsets.only(top: SizeConfig.safeBlockVertical * 1),
                                                              child: Column(
                                                                mainAxisAlignment: MainAxisAlignment.start,
                                                                        ...
                                                                        delegate: SliverChildBuilderDelegate(
                                                                          (context, index) {
                                                                            ...
                                                                            order.itemList.forEach((element) {
                                                                              snapshot.serviceList.serviceListState.forEach((s) {
                                                                                if (element.id == s.serviceId) service = s;
                                                                              });
                                                                            });
                                              ...
                                              serviceSnapshot.data.forEach((serviceState) {
                                                ...
                                                searchRecommended(Provider.of<Explorer>(context, listen: false).serviceList);*/
                                                              ...
                                                              StreamBuilder<QuerySnapshot>(
                                                                  ...
                                                                  //debugPrint('----------');
                                                                    ...
                                                                    categorySnaphot.data.docs.forEach((element) {
                                                                      ...
                                                                      if (category.level == 0) {
                                                                        //debugPrint('RUI_U_service_explorer => NOME CATEGORIA => ${category.name} e SERVICECOUNT => ${category.serviceCount}');
                                                                    ...
                                                                    tmpRoots.forEach((category) {
                                                                      ...
                                                                      if (category.activeServiceCount == 0) {
                                                                        tmpNonRoots.forEach((categoryNonRoots) {
                                                                          if (categoryNonRoots.parent.id == category.id) {
                                                                            sub = true;
                                                                            //debugPrint('SUB CATEGORY: ${categoryNonRoots.name} OF ${category.name} - SERVICE COUNT: ${categoryNonRoots.serviceCount} - ACTIVE SERVICE COUNT: ${categoryNonRoots.activeServiceCount}');
                                                                            if (categoryNonRoots.activeServiceCount == 0) {
                                                                              remove = true;
                                                                            } else {
                                                                              Provider.of<Explorer>(context, listen: false).allCategoryList.add(categoryNonRoots);
                                                                            }
                                                                          }
                                                                        });
                                                                        if (sub && remove) {
                                                                        ...
                                                                        SliverList(
                                                                          delegate: SliverChildBuilderDelegate(
                                                                            (context, index) {
                                                                              ...
                                                                              //debugPrint('RUI_U_service_explorer => CATEGORY => ${category.name}');
                                                      ...
                                                      if (Provider.of<Explorer>(context, listen: false).businessState != null && Provider.of<Explorer>(context, listen: false).businessState.id_firestore.isNotEmpty) {
                                                        childrens.add(StreamBuilder<QuerySnapshot>(
                                                            stream: FirebaseFirestore.instance
                                                                .collection("service")
                                                                .where("visibility", isEqualTo: 'Active')
                                                            ...
                                                            builder: (context, AsyncSnapshot<QuerySnapshot> categorySnapshot) {
                                                              if (categorySnapshot.hasError || categorySnapshot.connectionState == ConnectionState.waiting) {
                                                                //debugPrint('RUI_U_service_explorer => Hub Services - CATEGORY WAITING');
                                                                return Flexible(
                                                                  child: Container(
                                                                    margin: EdgeInsets.only(top: SizeConfig.safeBlockVertical * 0),
                                                                    padding: EdgeInsets.only(bottom: SizeConfig.safeBlockVertical * 2),
                                                              ...
                                                              return tmpHubServices.length >= 0 //TODO -> Decidere se farli vedere sempre o no!!! Metto a 0 ma prima era a 4
                                                                  ? Flexible(
                                                                      child: Container(
                                                                        margin: EdgeInsets.only(top: SizeConfig.safeBlockVertical * 0),
                                                                        ...
                                                                        child: Column(
                                                                          ...
                                                                          children: [
                                                                                      ...
                                                                                      child: Material(
                                                                                        ...
                                                                                        child: InkWell(
                                                                                            onTap: () {
                                                                                              Navigator.push(
                                                                                                context,
                                                                                                MaterialPageRoute(builder: (context) => ViewMore(AppLocalizations.of(context).hubsTopServices, orderList, false, [], true)),
                                                                                              );
                                                                                            },
                                                                                            borderRadius: BorderRadius.all(Radius.circular(5.0)),
                                                                                            child: Container(
                                                                            ...
                                                                            Container(
                                                                              ...
                                                                              child: Text(
                                                                                '${AppLocalizations.of(context).discoverHub}',
                                                                                style: TextStyle(fontFamily: BuytimeTheme.FontFamily, color: BuytimeTheme.TextBlack.withOpacity(.6), fontWeight: FontWeight.w500, fontSize: 14
                                                                        ...
                                                                        child: Material(
                                                                          ...
                                                                          child: InkWell(
                                                                              onTap: () {
                                                                                Navigator.push(
                                                                                  context,
                                                                                  MaterialPageRoute(builder: (context) => ViewMore(AppLocalizations.of(context).popular, orderList, true, [], false)),
                                                                                );
                                                                              },
                                                                              borderRadius: BorderRadius.all(Radius.circular(5.0)),
                                                                              child: Container(
                                                                                  ...
                                                                                  builder: (context, AsyncSnapshot<bool> businessSnapshot) {
                                                                                    ...
                                                                                    return businessSnapshot.data
                                                                                        ? Container(
                                                                                            ...
                                                                                            decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.all(Radius.circular(25)), border: Border.all(color: Colors.black, width: .5)),
                                                    ...
                                                    Container(
                                                      ...
                                                      Provider.of<Explorer>(context, listen: false).rootCategoryList.forEach((category) async {
                                                        ...
                                                        if (enoughServices) {
                                                          ...
                                                          if (StoreProvider.of<AppState>(context).state.area != null && StoreProvider.of<AppState>(context).state.area.areaId != null && StoreProvider.of<AppState>(context).state.area.areaId.isNotEmpty) {
                                                            thisCategory = FirebaseFirestore.instance
                                                                .collection("service")
                                                            ...
                                                            thisCategory = FirebaseFirestore.instance.collection("service").where("visibility", isEqualTo: 'Active').where('categoryId', arrayContainsAny: category.categoryIdList.length > 10 ? category.categoryIdList.sublist(0, 10) : category.categoryIdList).limit(4).snapshots(includeMetadataChanges: true);
                                                          }
                                                                                        ...
                                                                                        child: Material(
                                                                                          ...
                                                                                          child: InkWell(
                                                                                              onTap: () {
                                                                                                Navigator.push(
                                                                                                  context,
                                                                                                  MaterialPageRoute(builder: (context) => ViewMore(category.name, orderList, false, category.categoryIdList, false)),
                                                                                                );
                                                                                              },
                                                                                              borderRadius: BorderRadius.all(Radius.circular(5.0)),
                                                                                              child: Container(
                                                                          ...
                                                                          Expanded(
                                                                              ...
                                                                              child: GestureDetector(
                                                                                ...
                                                                                child: Container(
                                                                                  ...
                                                                                  width: 24,
                                                                                  decoration: BoxDecoration(
                                                                                    image: DecorationImage(image: AssetImage('assets/img/whatsapp.png'), fit: BoxFit.contain),
                                                                                  ),
                                                                                ),
                                                                              ))
                                                                        ],
                                                      ...
                                                      ///Log out
                                                      childrens.add(FirebaseAuth.instance.currentUser != null && FirebaseAuth.instance.currentUser.uid.isNotEmpty /*&& cards.isNotEmpty*/ ? Container(
                                                              color: Colors.white,
                                                              height: 64,
                                                              child: Material(
                                                                color: Colors.transparent,
                                                          ...
                                                          child: CustomScrollView(physics: new ClampingScrollPhysics(), shrinkWrap: true, scrollDirection: Axis.vertical, slivers: [
                                                            SliverList(
                                                              delegate: SliverChildBuilderDelegate(
                                                                (context, index) {
                                                                  //MenuItemModel menuItem = menuItems.elementAt(index);
                                                                  if (Provider.of<Explorer>(context, listen: false).searchedList.length > 1 || (Provider.of<Explorer>(context, listen: false).searchedList.length == 1 && Provider.of<Explorer>(context, listen: false).searchedList.first.isNotEmpty)) {
                                                                    if (index == 0) {
                                                                      return Container(
